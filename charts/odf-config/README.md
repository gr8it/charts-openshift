...
`StorageCluster`/`StorageSystem`/`LocalVolumeDiscovery`/`LocalVolumeSet` and the device sets they imply are fully owned by this chart (normal apply, not server-side); only `storageCluster.name`, `nodeCount`, `localVolumeSetCount`, `deviceMinSize`, `resources` and `cephCluster` are configurable, everything else follows the fixed spec observed across existing ODF clusters. `deviceMinSize` defaults to `6000Gi` (the disk size on all clusters at the time of writing) — override it in the conf repo for clusters with different disks.
